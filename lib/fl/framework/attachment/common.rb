...
module Fl::Framework::Attachment
  ...
  module Common
    ...
    class AttachableAccessValidator < ActiveModel::Validator
      ...
      # If *record* is associated with an attachable, it must respond to the `:permission?` method.
    ...
    module ClassMethods
      ...
      #  *actor* can list attachments if it has read access to the attachable.
      ...
      #  *actor* can add attachments if it has write access to the attachable.
    ...
    module InstanceMethods
      ...
      #  *actor* can read the attachment if it has read access to the attachable.
      ...
      # @param context The context in which to do the check.
      #
      # @return [Symbol, nil, Boolean] Returns `:private` if *actor* is the same as the attachment's author,
      ...
      #
      # @return [Symbol, nil, Boolean] Forwards the call to the attachable, using the **:read** permission:
      #  *actor* can download the attachment file if it has read access to the attachable.
    ...
    #
    # - Registers the methods in {ClassMethods} as class methods of *base*.
    # - In the context of the *base* (and therefore of the comment class), includes the modules
    ...
    #   Because the includes are executed in the *base*, the functionality is loaded in the comment
