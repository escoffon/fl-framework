module Fl::Framework::Attachment::Neo4j
  ...
  # - *:create* permission is granted if the user has +:write+ access to the context (which is the potential master).
  ...
  # This implies that the master object must also implement the {Fl::Framework::Access::Access} interface.
  #
  # === Properties
  ...
  class Base
    ...
    has_one :out, :master, rel_class: :'Fl::Framework::Rel::Attachment::AttachedTo'
    ...
    def _read_check(op, obj, user, context = nil)
      obj.master.permission?(user, Fl::Framework::Access::Grants::READ, context)
    end

    def _write_check(op, obj, user, context = nil)
      obj.master.permission?(user, Fl::Framework::Access::Grants::WRITE, context)
    end
